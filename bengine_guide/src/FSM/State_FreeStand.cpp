...
void State_FreeStand::enter(){
    ...
    _initVecOX = _ctrlComp->robotModel->getX(*_lowState);           //获取右前腿的足端坐标（身体坐标系）
    _initVecXP = _ctrlComp->robotModel->getVecXP(*_lowState);       //获取另外三条腿的足端坐标-右前腿坐标（身体坐标系）
