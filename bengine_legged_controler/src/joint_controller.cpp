...
namespace bengine_legged_controller {
    ...
    BengineJointController::BengineJointController(){}

    BengineJointController::~BengineJointController(){
        sub_cmd.shutdown();
    }

    void BengineJointController::setCommandCB(const bengine_legged_msgs::MotorCmdConstPtr& msg)
    {
    ...
    bool BengineJointController::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
    {
        ...
        sub_cmd = n.subscribe("command", 20, &BengineJointController::setCommandCB, this);
 
        // Start realtime state publisher
        controller_state_publisher_.reset(
            new realtime_tools::RealtimePublisher<bengine_legged_msgs::MotorState>(n, name_space + "/state", 1));        
        return true;
    }

    void BengineJointController::setGains(const double &p, const double &i, const double &d, const double &i_max, const double &i_min, const bool &antiwindup)
    {
        pid_controller_.setGains(p,i,d,i_max,i_min,antiwindup);
    }

    void BengineJointController::getGains(double &p, double &i, double &d, double &i_max, double &i_min, bool &antiwindup)
    {
        pid_controller_.getGains(p,i,d,i_max,i_min,antiwindup);
    }

    void BengineJointController::getGains(double &p, double &i, double &d, double &i_max, double &i_min)
    {
        bool dummy;
    ...
    void BengineJointController::starting(const ros::Time& time)
    {
    ...
    void BengineJointController::update(const ros::Time& time, const ros::Duration& period)
    {
    ...
    void BengineJointController::stopping(){}

    void BengineJointController::positionLimits(double &position)
    {
    ...
    void BengineJointController::velocityLimits(double &velocity)
    {
        if (joint_urdf->type == urdf::Joint::REVOLUTE || joint_urdf->type == urdf::Joint::PRISMATIC)
            clamp(velocity, -joint_urdf->limits->velocity, joint_urdf->limits->velocity);
    }

    void BengineJointController::effortLimits(double &effort)
    {
        if (joint_urdf->type == urdf::Joint::REVOLUTE || joint_urdf->type == urdf::Joint::PRISMATIC)
...
PLUGINLIB_EXPORT_CLASS(bengine_legged_controller::BengineJointController, controller_interface::ControllerBase);
